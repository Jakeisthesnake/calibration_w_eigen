...
std::vector<CSVRow> readCSV(const std::string& file_path) {
    ...
    while (std::getline(file, line)) {
        ...
        std::getline(ss, token, ',');
        row.cam_id = std::stoi(token);

        std::getline(ss, token, ',');
        row.corner_x = std::stod(token);

        std::getline(ss, token, ',');
        row.corner_y = std::stod(token);
...
> processCSV(const std::string& file_path, int target_cam_id)
{
    ...
    for (const auto& row : rows) {
        if (row.cam_id != target_cam_id) continue;

        Eigen::Vector2d img_pt(row.corner_x, row.corner_y);
    ...
    // std::cout << "Processed " << obj_pts_list.size() << " timestamps." << std::endl;
...
struct RelativePoseErrorQuat {
                    ...
                    T* residuals) const
    {
        ...
        for (int i = 0; i < 3; ++i) {
            residuals[i]     = rvec_err[i];
            residuals[i + 3] = T_err[i];
        }
    ...
    const double* dist_1)
{
    ...
    // Assume same fx/fy/cx/cy for both cameras
    int img_w = static_cast<int>(2 * intrinsic_0[2]);
    int img_h = static_cast<int>(2 * intrinsic_0[3]);

    pangolin::OpenGlRenderState s_cam(
        pangolin::ProjectionMatrixOrthographic(0, 2 * img_w, img_h, 0, -1, 1)
    );

    pangolin::View& d_cam = pangolin::CreateDisplay()
        .SetBounds(0.0, 1.0, 0.0, 1.0)
        .SetAspect(static_cast<float>(2 * img_w) / img_h)
    ...
    pangolin::RegisterKeyPressCallback('a', [&]() {
        ...
        std::cout << "Frame error: " 
                  ...
                  << (map_cam1.count(ts) ? map_cam1[ts].error_sum : 0.0) << " (Cam1)" << std::endl;
...
int main(int argc, char** argv) {
    ...
    // Step 1: Load and process CSV data for both cameras
    ...
    // Step 3: Filter data by timestamps
    ...
    // Eigen::Matrix3d K_0 = robust_intrinsic_estimation(H_list_0, 30, 5);
    // Eigen::Matrix3d K_1 = robust_intrinsic_estimation(H_list_1, 30, 5);

    // std::cout << "Intrinsic Matrix Camera 0:\n" << K_0 << std::endl;
    // std::cout << "Intrinsic Matrix Camera 1:\n" << K_1 << std::endl;

    // Eigen::Matrix3d K_0 = compute_intrinsic_params(H_list_0);
    // Eigen::Matrix3d K_1 = compute_intrinsic_params(H_list_1);


    // double intrinsic_0[4] = {800, 800, 640, 480}; // fx, fy, cx, cy
    // double intrinsic_1[4] = {810, 810, 650, 470};
    ...
    // Step 5: Compute extrinsic parameters
    std::vector<std::array<double, 7>> extrinsics_0, extrinsics_1;
    ...
    // Step 6: Initialize distortion coefficients and inter-camera transformation
    // double intrinsic_0[4] = {800, 800, 640, 480}; // fx, fy, cx, cy
    // double dist_0[4] = {0.1, -0.05, 0.02, -0.01};
    // double intrinsic_1[4] = {810, 810, 650, 470};
    // double dist_1[4] = {0.09, -0.04, 0.015, -0.008};
    // double rvec_cam_1[3] = {0.0, 0.0, 0.0};
    // double tvec_cam_1[3] = {0.1, 0.0, 0.0}; // Baseline of 10 cm
    ...
    // double intrinsic_0[4] = {800, 800, 640, 480}; // fx, fy, cx, cy
    // double dist_0[4] = {-0.45, 0.17, -0.03, 0.004};
    // double intrinsic_1[4] = {810, 810, 650, 470};
    // double dist_1[4] = {-0.45, 0.17, -0.03, 0.004};
    // double rvec_cam_1[3] = {0.0, 0.0, 0.0};
    // double tvec_cam_1[3] = {0.1, 0.0, 0.0}; // Base0.09, -0.04, 0.015, -0.008line of 10 cm

    double intrinsic_0[4] = {K_0(0, 0), K_0(1, 1), K_0(0, 2), K_0(1, 2)};
    double dist_0[4] = {0.1, -0.05, 0.02, -0.01}; 
    double intrinsic_1[4] = {K_1(0, 0), K_1(1, 1), K_1(0, 2), K_1(1, 2)};
    double dist_1[4] = {0.1, -0.05, 0.02, -0.01}; 
    double rvec_cam_1[3] = {0.1, 0.1, 0.1};
    double tvec_cam_1[3] = {0.1, 0.0, 0.0}; // Base0.09, -0.04, 0.015, -0.008line of 10 cm
    ...
    // auto frames_cam0 = GenerateReprojectionErrorData(
    ...
    // auto frames_cam1 = GenerateReprojectionErrorData(
    //     intrinsic_1, dist_1, extrinsics_1, img_pts_list_1, obj_pts_list_1, filtered_timestamp_list_1);
    
    // VisualizeStereoReprojectionError(
    //     frames_cam0, frames_cam1,
    //     intrinsic_0, dist_0,
    //     intrinsic_1, dist_1);
    ...
    // std::cout << "Img pts list 0 size: " << img_pts_list_0.size() << std::endl;
    ...
    auto cam1_data = GenerateReprojectionErrorData(
        intrinsic_1, dist_1, extrinsics_1, img_pts_list_1, obj_pts_list_1, filtered_timestamp_list_1);
    // TestPangolinWindow();
    // Visualize one or both
    // VisualizeReprojectionError(cam0_data, cam1_data, intrinsic_0, dist_0, intrinsic_1, dist_1);
    // VisualizeReprojectionError(cam1_data, intrinsic_1, dist_1);
    VisualizeStereoReprojectionError(cam0_data, cam1_data, intrinsic_0, dist_0, intrinsic_1, dist_1);
    ...
    std::cout << "Inter-camera Rotation Vector: ";
    for (double r : rvec_cam_1) std::cout << r << " ";
    std::cout << "\nInter-camera Translation Vector: ";
